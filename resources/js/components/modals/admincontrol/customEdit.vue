...
<el-form label-position="top" label-width="100%">
  <el-form-item label="Branch Limit">
    <el-select
    ...
    >
      <!-- Read-only display of current expiration -->
      <el-option
        v-if="form.branch_limit"
        :label="`Current: ${form.branch_limit}`"
        value=""
        disabled
      />
...
const props = defineProps({
  ...
  selectedAccount: {
    type: Object,
    default: () => ({ id: null, name: '', email: '' ,subscription_expires_at: '',branch_limit: '' })
  }
});
...
watch(
  ...
  (newVal) => {
    if (newVal && props.form) {
      ...
      props.form.branch_limit = newVal.branch_limit || '';
