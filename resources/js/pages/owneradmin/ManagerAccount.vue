...
import UserModal from '@/components/modals/owneradmin/AddAccountModal.vue';
...
const breadcrumbs: BreadcrumbItem[] = [
  {
    ...
    href: '/owneradmin/ManagerAccount',
  },
];
...
function confirmDelete() {
  if (selectedId.value !== null) {
    form.delete(route('owneradmin.ManagerAccount.destroy', selectedId.value), {
      preserveScroll: true,
    });
...
<template>
  <Head title="Manager Account" />
  <Suspense>
        <template #default>
         <AppLayout :breadcrumbs="breadcrumbs">
          <UserModal :branches="props.branches" :form="form" />
             <DeleteModal
...
/>
 <div class="m-8">
            ...
            <div class="absolute inset-y-0 left-1 flex items-center pl-3 pointer-events-none">
              <svg
                ...
                <input
                 ...
                 placeholder="Search for Manager Account"
                />
              </div>
             ...
             Branch Names
              ...
              <th
                ...
                >
                  ...
                  >
                    <tr
               v-for="(managerAccount, index) in props.managers.data" :key="managerAccount.id"
                      ...
                      >
                                {{ (props.managers.from || 0) + index }}
                      </td>
                                              <td class="p-5 whitespace-nowrap">
    <img
      :src="managerAccount.profile || LogoImage"
      alt="Profile"
   ...
   <td class="p-5 whitespace-nowrap text-sm leading-6 font-semibold text-orange-600 bg-orange-100">
      ...
      {{ managerAccount.branchNames.join(',  ') }}
</td>
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                      :style="{ color: 'var(--table-text)' }" >
                      ...
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                      :style="{ color: 'var(--table-text)' }" >
                        {{ formatDate(managerAccount.created_at) }}
                      </td>
                      <td class="p-5">
                        <div class="flex items-center gap-1">
                          ...
                          <button
                   @click="showDeleteModal(managerAccount.id)"
