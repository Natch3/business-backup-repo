...
import UserModal from '@/components/modals/owneradmin/AddBrachesModal.vue';
import DeleteModal from '@/components/modals/owneradmin/customDelete.vue';
import EditModal from '@/components/modals/owneradmin/customEditRegisterBusiness.vue';

import { ElMessage } from 'element-plus';
import { ChevronUpIcon, ChevronDownIcon  } from '@heroicons/vue/20/solid';

// Lazy-load the layout to enable suspense loading
const AppLayout = defineAsyncComponent(() => import('@/layouts/owneradmin/AppLayout.vue'));

const breadcrumbs: BreadcrumbItem[] = [
  {
    title: 'Register Business',
    href: '/owneradmin/RegisterBusiness',
  },
];
...
function confirmDelete() {
  if (selectedId.value !== null) {
    form.delete(route('owneradmin.RegisterBusiness.destroy', selectedId.value), {
      preserveScroll: true,
    });
...
<button
  ...
  class="flex items-center justify-center cursor-pointer gap-2 
         px-3 sm:px-5 py-2.5 
         rounded-smcursor-pointer 
         text-slate-900 text-sm font-medium outline-none bg-transparent rounded-md border border-gray-300  dark:text-white"
>
...
<ul
  ...
  class="absolute right-0 rounded-sm shadow-lg py-2 z-[1000] min-w-full w-max 
         divide-y divide-gray-200 max-h-96 overflow-auto 
         dark:divide-gray-700 dark:text-white 
         border border-gray-200 dark:border-gray-700 mt-2"
  :style="{ backgroundColor: 'var(--background)' }"
>
  <li class="px-5 py-2.5 text-slate-600 dark:text-gray-300 text-sm font-medium cursor-pointer hover:text-blue-600 dark:hover:text-blue-400">
    Dropdown option
  </li>
  <li class="px-5 py-2.5 text-slate-600 dark:text-gray-300 text-sm font-medium cursor-pointer hover:text-blue-600 dark:hover:text-blue-400">
    Cloth set
              ...
              <tr :style="{ backgroundColor: 'var(--table-header-bg)',
                ...
                >
                 ...
                 <th
                  ...
                  :style="{ color: 'var(--table-header-text)' }"
                >
                <div class="flex items-center gap-1 cursor-pointer" @click="handleSort('branch_name')">
    Branch Name
    <ChevronUpIcon
      v-if="sortBy === 'branch_name' && sortDirection === 'asc'"
      class="w-4 h-4 text-gray-500"
    />
    <ChevronDownIcon
      v-if="sortBy === 'branch_name' && sortDirection === 'desc'"
      class="w-4 h-4 text-gray-500"
    />
  </div>
                  </th>
                  ...
                  :style="{ color: 'var(--table-header-text)' }"
                >
                 <div class="flex items-center gap-1 cursor-pointer" @click="handleSort('business_type')">
    Branch Type
    <ChevronUpIcon
      v-if="sortBy === 'business_type' && sortDirection === 'asc'"
      class="w-4 h-4 text-gray-500"
    />
    <ChevronDownIcon
      v-if="sortBy === 'business_type' && sortDirection === 'desc'"
      class="w-4 h-4 text-gray-500"
    />
  </div>
                  </th>
                ...
                >
                   <div class="flex items-center gap-1 cursor-pointer" @click="handleSort('status')">
                    Status
    <ChevronUpIcon
      v-if="sortBy === 'status' && sortDirection === 'asc'"
      class="w-4 h-4 text-gray-500"
    />
    <ChevronDownIcon
      v-if="sortBy === 'status' && sortDirection === 'desc'"
      class="w-4 h-4 text-gray-500"
    />
  </div>
                  </th>
               <th
                  ...
                  :style="{ color: 'var(--table-header-text)' }"
                >
                 <div class="flex items-center gap-1 cursor-pointer" @click="handleSort('created_at')">
                    Created  At
    <ChevronUpIcon
    ...
    />
              ...
              <th
                ...
                >
                  ...
                  >
                    <tr
             v-for="(branchesOwner, index) in props.branches.data" :key="branches.id"
                      ...
                      >
                        {{ (props.branches.from || 0) + index }}
                      </td>
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                       :style="{ color: 'var(--table-text)' }"
                        >
                          {{ branchesOwner.branch_name }}
                      </td>
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                      :style="{ color: 'var(--table-text)' }" >
                                     {{ branchesOwner.business_type }}
                      </td>
<td
  class="p-5 whitespace-nowrap text-sm leading-6 font-semibold"
  :class="{
    'text-green-600 bg-green-200': branchesOwner.status === 'Approved',
    'text-red-600 bg-red-100': branchesOwner.status === 'Rejected',
    'text-orange-500 bg-orange-100': branchesOwner.status === 'Pending'
  }"
>
  {{ branchesOwner.status }}
</td>

                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                      :style="{ color: 'var(--table-text)' }" >
              {{ formatDate(branchesOwner.created_at) }}
                      </td>
                      <td class="p-5">
                        <div class="flex items-center gap-1">
                          <button
                     @click="showEditModal(branchesOwner)"
                          ...
                          <button
                           @click="showDeleteModal(branchesOwner.id)"
                            class="p-2 rounded-full group transition-all duration-500 flex item-center cursor-pointer"
                          >
                            <svg
                              class=""
                              width="20"
...
<div class="flex items-center space-x-2 mb-4">
  ...
  >
    ...
    <div class="flex space-x-1">
      ...
      <!-- First -->
<button
  @click="goTo(branches.first_page_url)"
  :disabled="!branches.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    branches.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  First
</button>
...
<!-- Prev -->
<button
  @click="goTo(branches.prev_page_url)"
  :disabled="!branches.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    branches.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
...
<!-- Next -->
<button
  @click="goTo(branches.next_page_url)"
  :disabled="!branches.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    branches.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  Next
</button>
...
<!-- Last -->
<button
  @click="goTo(branches.last_page_url)"
  :disabled="!branches.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    branches.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
