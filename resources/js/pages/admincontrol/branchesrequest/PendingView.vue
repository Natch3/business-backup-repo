...
const breadcrumbs: BreadcrumbItem[] = [
  {
    title: 'Pending View',
    href: '/admincontrol/branchesrequest/PendingView',
  },
];
...
const filteredLinks = computed(() => {
  const links = props.branchViews.links || [];
...
/>
 ...
 <div class="m-8">
  ...
  <button
    @click="goBack"
    class="group flex items-center cursor-pointer gap-2 px-5 py-2 bg-white text-gray-800 font-medium rounded-sm border border-gray-300 hover:bg-gray-100 hover:border-gray-400 transition-all duration-300"
  >
    <!-- Arrow Left Icon -->
    ...
    <span class="group-hover:text-black">Back</span>
  </button>
</div>
...
<button
  ...
  class="flex items-center justify-center gap-2 
         px-3 sm:px-5 py-2.5 
         rounded-sm border border-gray-300 cursor-pointer 
         text-slate-900 text-sm font-medium outline-none bg-white hover:bg-gray-50"
>
            ...
            <thead>
              ...
              <th
                ...
                >
                  ...
                  >
                    <tr
              v-for="(branchView, index) in props.branchViews.data" :key="branchViews.id"
                      ...
                      >
                         {{ (props.branchViews.from || 0) + index }}
                      </td>
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                       :style="{ color: 'var(--table-text)' }"
  ...
  :class="{
    'text-green-600 bg-green-200 font-bold': branchView.status === 'Approved',
    'text-red-600 bg-red-100 font-bold': branchView.status === 'Rejected',
    'text-orange-500 bg-orange-100 font-bold': branchView.status === 'Pending'
  }"
>
                        {{ branchView.status }}
                      ...
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                      :style="{ color: 'var(--table-text)' }" >
{{ formatDate(branchView.user.created_at) }}

                      </td>
                      <td class="p-5">
                        <div class="flex items-center gap-1">
                               <button
                                @click="showEditModal(branchView)"
                            class="relative  p-2 rounded-full group transition-all duration-500 flex item-center"
                          >
                            <svg
...
<div class="flex items-center space-x-2 mb-4">
  ...
  >
    ...
    <div class="flex space-x-1">
      ...
      <!-- First -->
<button
  @click="goTo(branchViews.first_page_url)"
  :disabled="!branchViews.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    branchViews.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  First
</button>
...
<!-- Prev -->
<button
  @click="goTo(branchViews.prev_page_url)"
  :disabled="!branchViews.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    branchViews.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
...
<!-- Next -->
<button
  @click="goTo(branchViews.next_page_url)"
  :disabled="!branchViews.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    branchViews.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  Next
</button>
...
<!-- Last -->
<button
  @click="goTo(branchViews.last_page_url)"
  :disabled="!branchViews.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    branchViews.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
