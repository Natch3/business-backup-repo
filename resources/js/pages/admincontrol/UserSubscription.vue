...
import EditModal from '@/components/modals/admincontrol/customEdit.vue';
import { ChevronUpIcon, ChevronDownIcon  } from '@heroicons/vue/20/solid';

const AppLayout = defineAsyncComponent(() => import('@/layouts/admincontrol/AppLayout.vue'));

const breadcrumbs: BreadcrumbItem[] = [
  { title: 'UserSubscription', href: '/admincontrol/UserSubscription' },
];
...
const filteredLinks = computed(() => {
  const links = props.ownerAdmins.links || [];
...
function confirmDelete() {
  if (selectedId.value !== null) {
    form.delete(route('admincontrol.UserSubscription.destroy', selectedId.value), {
      preserveScroll: true,
    });
    ElMessage.success("User Account Deleted Successfully!");
...
const selectedAccount = ref({
  ...
  branch_limit: null as number | null,
...
function confirmEdit() {
  ...
  if (selectedAccount.value.id !== null) {
    form.put(route('admincontrol.UserSubscription.update', selectedAccount.value.id), {
      preserveScroll: true,
      onSuccess: () => {
        ElMessage.success("User Account Updated Successfully!");
        showEditModalFlag.value = false;
        selectedAccount.value = { id: null, name: null, email: null, role: null, subscription_expires_at: null, branch_limit: null };
      },
    });
  }
}
...
function formatDate(dateString) {
  if (!dateString) return 'Not Subscribed yet';
...
<th
  ...
  :style="{ color: 'var(--table-header-text)' }"
>
  <div class="flex items-center gap-1 cursor-pointer" @click="handleSort('email')">
    Email
    <ChevronUpIcon
      v-if="sortBy === 'email' && sortDirection === 'asc'"
      class="w-4 h-4 text-gray-500"
    />
    <ChevronDownIcon
      v-if="sortBy === 'email' && sortDirection === 'desc'"
      class="w-4 h-4 text-gray-500"
    />
  </div>
</th>
  ...
  :style="{ color: 'var(--table-header-text)' }"
>
  <div class="flex items-center gap-1 cursor-pointer" @click="handleSort('role')">
  User Type
    <ChevronUpIcon
      v-if="sortBy === 'role' && sortDirection === 'asc'"
      class="w-4 h-4 text-gray-500"
    />
    <ChevronDownIcon
      v-if="sortBy === 'role' && sortDirection === 'desc'"
      class="w-4 h-4 text-gray-500"
    />
  </div>
</th>  
              ...
              <th
                ...
                >
                  ...
                  >
                    ...
                    <tr
 v-for="(ownerAdmin, index) in props.ownerAdmins.data" :key="ownerAdmin.id"
                      ...
                      >
           {{ (props.ownerAdmins.from || 0) + index }}
                           
                      </td>
     <td class="p-5 whitespace-nowrap">
  <img
    :src="ownerAdmin?.avatar ? '/storage/' + ownerAdmin.avatar : LogoImage"
    alt="Profile"
...
<td
  ...
  :class="{
    'text-green-600 bg-green-200': ownerAdmin.role === 'OwnerAdmin',
    'text-orange-600 bg-orange-100': ownerAdmin.role === 'Users'
  }"
>
                        {{ ownerAdmin.role }}
           ...
           <td
  class="p-5 whitespace-nowrap text-sm leading-6 font-medium rounded"
  :class="{
    'text-red-600 font-bold bg-red-50': new Date(ownerAdmin.subscription_expires_at) < new Date(),
    'text-green-700 font-semibold bg-green-200': new Date(ownerAdmin.subscription_expires_at) >= new Date()
  }"
>
  <span v-if="new Date(ownerAdmin.subscription_expires_at) < new Date()">
    {{ formatDate(ownerAdmin.subscription_expires_at) }} - Expired
  </span>
  <span v-else>
    {{ formatDate(ownerAdmin.subscription_expires_at) }} - Active
  </span>
</td>
                      ...
                      <td class="p-5">
                        <div class="flex items-center gap-1">
                          <button
                                @click="showEditModal(ownerAdmin)"
                          ...
                          <button
                             @click="showDeleteModal(ownerAdmin.id)"
...
<div class="flex items-center space-x-2 mb-4">
  ...
  >
    ...
    <div class="flex space-x-1">
      ...
      <!-- First -->
<button
  @click="goTo(ownerAdmins.first_page_url)"
  :disabled="!ownerAdmins.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    ownerAdmins.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  First
</button>
...
<!-- Prev -->
<button
  @click="goTo(ownerAdmins.prev_page_url)"
  :disabled="!ownerAdmins.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    ownerAdmins.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
...
<!-- Next -->
<button
  @click="goTo(ownerAdmins.next_page_url)"
  :disabled="!ownerAdmins.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    ownerAdmins.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  Next
</button>
...
<!-- Last -->
<button
  @click="goTo(ownerAdmins.last_page_url)"
  :disabled="!ownerAdmins.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    ownerAdmins.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
