...
import RestaurantModal from '@/components/modals/manager/InventoryRestaurant.vue';
...
const breadcrumbs: BreadcrumbItem[] = [
  {
    title: 'View Inventory Restaurant',
    href: '/manager/InventoryList/ViewInventoryRestaurant',
  },
];
...
function confirmDelete() {
  if (selectedId.value !== null) {
    form.delete(route('manager.InventoryList.ViewInventoryRestaurant.destroy', selectedId.value), {
      preserveScroll: true,
    });
...
<div class="mt-4 ml-8 flex justify-between items-center">
  ...
  <RestaurantModal :branch-id="branch.id" v-if="branch?.business_type === 'Restaurant'" :branch="branch" />
            ...
            <thead>
              ...
              <th
                ...
                >
                  ...
                  >
                    <tr
              v-for="(Inventoryrestaurants, index) in props.restaurants.data" :key="Inventoryrestaurants.id"
                      ...
                      >
                                      {{ Inventoryrestaurants.menu_id   }}
                      </td>
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                       :style="{ color: 'var(--table-text)' }"
                        >
                      {{ Inventoryrestaurants.menu_name   }}
                    
                      </td>
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                      :style="{ color: 'var(--table-text)' }" >
                   {{ Inventoryrestaurants.menu_category   }}
                    
                      </td>
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                      :style="{ color: 'var(--table-text)' }" >
                   {{ Inventoryrestaurants.menu_price   }}
                      </td>
                      <td class="p-5">
                        <div class="flex items-center gap-1">
                          ...
                          <button
                               @click="showDeleteModal(Inventoryrestaurants.id)"
...
<div class="flex items-center space-x-2 mb-4">
  ...
  >
    ...
    <div class="flex space-x-1">
      ...
      <!-- First -->
<button
  @click="goTo(restaurants.first_page_url)"
  :disabled="!restaurants.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    restaurants.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  First
</button>
...
<!-- Prev -->
<button
  @click="goTo(restaurants.prev_page_url)"
  :disabled="!restaurants.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    restaurants.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
...
<!-- Next -->
<button
  @click="goTo(restaurants.next_page_url)"
  :disabled="!restaurants.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    restaurants.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  Next
</button>
...
<!-- Last -->
<button
  @click="goTo(restaurants.last_page_url)"
  :disabled="!restaurants.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    restaurants.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
