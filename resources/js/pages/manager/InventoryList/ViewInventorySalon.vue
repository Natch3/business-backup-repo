...
import SalonModal from '@/components/modals/manager/InventorySalon.vue';

import DeleteModal from '@/components/modals/manager/customDelete.vue';
...
const breadcrumbs: BreadcrumbItem[] = [
  {
    title: 'View Inventory Salon',
    href: '/manager/InventoryList/ViewInventorySalon',
  },
];
...
const currentBusinessType = computed(() => {
  return props.branch?.business_type ?? null;
});
...
const filteredLinks = computed(() => {
  const links = props.salons.links || [];
...
function confirmDelete() {
  if (selectedId.value !== null) {
    form.delete(route('manager.InventoryList.ViewInventorySalon.destroy', selectedId.value), {
      preserveScroll: true,
    });
...
<template>
  ...
  <div class="mt-4 ml-8 flex justify-between items-center">
  <!-- Back Button -->
  <button
    @click="goBack"
    class="group flex items-center cursor-pointer gap-2 px-4 py-2 bg-white text-gray-800 font-medium rounded-sm border border-gray-300 hover:bg-gray-100 hover:border-gray-400 transition-all duration-300"
  >
    <!-- Arrow Left Icon -->
    <svg xmlns="http://www.w3.org/2000/svg" 
         class="h-5 w-5 text-gray-600 group-hover:text-black transition-colors duration-300" 
         fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
    </svg>
    <span class="group-hover:text-black text-sm">Back</span>
  </button>
    <SalonModal :branch-id="branch.id"  v-if="branch?.business_type === 'Salon'" :branch="branch" />
...
/>
<!-- Render the correct modal depending on business_type -->
   <div class="m-8">
            ...
            <thead>
              ...
              <th
                ...
                >
                  ...
                  >
                    <tr
             v-for="(Inventorysalons, index) in props.salons.data" :key="Inventorysalons.id"
                      ...
                      >
                        {{ Inventorysalons.service_id   }}
                      </td>
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                       :style="{ color: 'var(--table-text)' }"
                        >
                                 {{ Inventorysalons.service_name   }}
                      </td>
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                      :style="{ color: 'var(--table-text)' }" >
                              {{ Inventorysalons.service_category   }}
                      </td>
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                      :style="{ color: 'var(--table-text)' }" >
                        {{ Inventorysalons.service_price   }}
                      </td>
                      <td class="p-5">
                        <div class="flex items-center gap-1">
                          ...
                          <button
                          @click="showDeleteModal(Inventorysalons.id)"
...
<div class="flex items-center space-x-2 mb-4">
  ...
  >
    ...
    <div class="flex space-x-1">
      ...
      <!-- First -->
<button
  @click="goTo(salons.first_page_url)"
  :disabled="!salons.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    salons.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  First
</button>
...
<!-- Prev -->
<button
  @click="goTo(salons.prev_page_url)"
  :disabled="!salons.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    salons.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
...
<!-- Next -->
<button
  @click="goTo(salons.next_page_url)"
  :disabled="!salons.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    salons.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  Next
</button>
...
<!-- Last -->
<button
  @click="goTo(salons.last_page_url)"
  :disabled="!salons.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    salons.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
