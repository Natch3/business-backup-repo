...
import DeleteModal from '@/components/modals/customDelete.vue';
...
function confirmDelete() {
  if (selectedId.value !== null) {
    form.delete(route('manager.Inventory.destroy', selectedId.value), {
      preserveScroll: true   
    })
    ElMessage.success("Post Delete Successfully!");
...
<template>
  ...
  >
    ...
    <span class="group-hover:text-black">Back</span>
      ...
      <DeleteModal :visible="showModal" @confirm="confirmDelete" @cancel="cancelDelete" />
    <!-- Render the correct modal depending on business_type -->
    <SalonModal v-if="branch?.business_type === 'Salon'" :branch="branch" />
    <RetailModal v-if="branch?.business_type === 'Retail'" :branch="branch" />
    <RestaurantModal v-if="branch?.business_type === 'Restaurant'" :branch="branch" />
