...
import RetailModal from '@/components/modals/manager/InventoryRetail.vue';
...
const breadcrumbs: BreadcrumbItem[] = [
  {
    title: 'View Inventory Retail',
    href: '/manager/InventoryList/ViewInventoryRetail',
  },
];
...
const filteredLinks = computed(() => {
  const links = props.retails.links || [];
...
function confirmDelete() {
  if (selectedId.value !== null) {
    form.delete(route('manager.InventoryList.ViewInventoryRetail.destroy', selectedId.value), {
      preserveScroll: true,
    });
...
<div class="mt-4 ml-8 flex justify-between items-center">
  ...
  <!-- Retail Modal aligned to right -->
  <RetailModal 
    :branch-id="branch.id" 
    v-if="branch?.business_type === 'Retail'" 
    :branch="branch" 
  />
</div>
            ...
            <thead>
              ...
              <th
                ...
                >
                  ...
                  >
                    <tr
        v-for="(Inventoryretails, index) in props.retails.data" :key="Inventoryretails.id"
                ...
                {{ Inventoryretails.product_id  }}
                      ...
                      <td
                       ...
                       :style="{ color: 'var(--table-text)' }"
                        >
                                              {{ Inventoryretails.product_name }}
                     
                      </td>
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                      :style="{ color: 'var(--table-text)' }" >
                                           {{ Inventoryretails.product_category }}
                     
                      </td>
                      <td
                        class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"
                      :style="{ color: 'var(--table-text)' }" >
            {{ Inventoryretails.product_price }}
                      </td>
                      <td class="p-5">
                        <div class="flex items-center gap-1">
                          ...
                          <button
                         @click="showDeleteModal(Inventoryretails.id)"
...
<div class="flex items-center space-x-2 mb-4">
  ...
  >
    ...
    <div class="flex space-x-1">
      ...
      <!-- First -->
<button
  @click="goTo(retails.first_page_url)"
  :disabled="!retails.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    retails.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  First
</button>
...
<!-- Prev -->
<button
  @click="goTo(retails.prev_page_url)"
  :disabled="!retails.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    retails.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
...
<!-- Next -->
<button
  @click="goTo(retails.next_page_url)"
  :disabled="!retails.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    retails.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  Next
</button>
...
<!-- Last -->
<button
  @click="goTo(retails.last_page_url)"
  :disabled="!retails.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    retails.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
