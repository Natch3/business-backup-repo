...
import UserModal from '@/components/modals/manager/AddAccountModal.vue';
...
const props = defineProps<{
  ...
  branches: { id: number; branch_name: string; business_type: string }[];
  staff: {
    data: any[];
    links: any[];
    current_page: number;
...
function confirmDelete() {
  if (selectedId.value !== null) {
    form.delete(route('manager.AddAccount.destroy', selectedId.value), {
      preserveScroll: true,
    });
...
/>
            ...
            <thead>
              ...
              <th
                ...
                >
                  ...
                  >
                    <tr
               v-for="(staffAccount, index) in props.staff.data" :key="staffAccount.id"
                      ...
                      >
                        {{ (props.staff.from || 0) + index }}
                      </td>
                                                          <td class="p-5 whitespace-nowrap">
    <img
      :src="staffAccount.profile || LogoImage"
      alt="Profile"
   ...
   <td class="p-5 whitespace-nowrap text-sm leading-6 font-semibold text-orange-600 bg-orange-100">
   
      {{ staffAccount.branchNames.join(',  ') }}
</td>
                      <td class="p-5">
                        <div class="flex items-center gap-1">
                          ...
                          <button
         @click="showDeleteModal(staffAccount.id)"
...
<div class="flex items-center space-x-2 mb-4">
  ...
  >
    ...
    <div class="flex space-x-1">
      ...
      <!-- First -->
<button
  @click="goTo(staff.first_page_url)"
  :disabled="!staff.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    staff.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  First
</button>
...
<!-- Prev -->
<button
  @click="goTo(staff.prev_page_url)"
  :disabled="!staff.prev_page_url"
  :class="[
    'px-2 py-1 border rounded',
    staff.prev_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
...
<!-- Next -->
<button
  @click="goTo(staff.next_page_url)"
  :disabled="!staff.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    staff.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
  ]"
>
  Next
</button>
...
<!-- Last -->
<button
  @click="goTo(staff.last_page_url)"
  :disabled="!staff.next_page_url"
  :class="[
    'px-2 py-1 border rounded',
    staff.next_page_url ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
